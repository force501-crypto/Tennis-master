...
from utils.losses import FocalSoftmaxCrossEntropyLoss, get_class_weights
...
flags.DEFINE_bool('horizontal_flip', True,
                  'Randomly flip RGB training images and swap left/right hit labels.')
flags.DEFINE_enum('class_weighting', 'none', ['none', 'inverse_sqrt', 'effective_num'],
                  'Class weighting method used by the classification loss.')
flags.DEFINE_float('class_weight_beta', 0.9999,
                   'Beta used by effective-number class weighting.')
flags.DEFINE_float('focal_gamma', 0.0,
                   'Focal loss gamma. Zero keeps standard cross entropy behaviour.')
...
def main(_argv):
    ...
    horizontal_flip_train = (FLAGS.horizontal_flip and FLAGS.feats_model is None
                             and not bool(FLAGS.flow) and not FLAGS.save_feats)
    ...
    if FLAGS.temp_pool not in ['max', 'mean']:
        train_set = TennisSet(split='train', transform=transform_train, every=FLAGS.every[0], padding=FLAGS.padding,
                              ...
                              save_feats=FLAGS.save_feats, horizontal_flip=horizontal_flip_train)
    ...
    if FLAGS.window > 1:  # Time Distributed RNN
        ...
        if FLAGS.temp_pool in ['gru', 'lstm']:
            ...
            logging.info('TCN temporal receptive field: %d frames', model.receptive_field)
    ...
    if FLAGS.feats_model is None:
        ...
        if FLAGS.temp_pool not in ['max', 'mean']:
            if FLAGS.feature_size > 0:
                ...
                logging.info('Detected stored feature window shape: %s',
                             tuple(summary_input.shape))
            logging.info(model.summary(summary_input))
    ...
    if FLAGS.temp_pool in ['max', 'mean']:
        ...
        class_weights = get_class_weights(class_counts, method=FLAGS.class_weighting,
                                          beta=FLAGS.class_weight_beta)
        logging.info('Training class counts: {}'.format(dict(zip(train_set.classes, class_counts))))
        logging.info('Loss class weights: {}'.format(
            dict(zip(train_set.classes, [round(float(w), 4) for w in class_weights]))))

        if FLAGS.class_weighting == 'none' and FLAGS.focal_gamma == 0:
            loss_fn = gluon.loss.SoftmaxCrossEntropyLoss()
        else:
            loss_fn = FocalSoftmaxCrossEntropyLoss(class_weights, gamma=FLAGS.focal_gamma)
