...
class TennisSet:
    def __init__(self, root='data', captions=False, transform=None, split='train', every=1, balance=True, padding=1,
                 ...
                 inference=False, feats_model=None, save_feats=False, horizontal_flip=False):
        self._root = root
    ...
    def flip_class(class_name):
        """Map left/right hit labels after a horizontal image flip."""
        return {
            'HFL': 'HFR',
            'HFR': 'HFL',
            'HNL': 'HNR',
    ...
    def __getitem__(self, idx):
        ...
        if self._captions:
            ...
            class_name = sample[2]
            flip_sample = self._horizontal_flip and not self._load_feats and random.random() < 0.5
            ...
            if flip_sample:
                class_name = self.flip_class(class_name)
