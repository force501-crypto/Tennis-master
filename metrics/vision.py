...
class PRF1(mx.metric.EvalMetric):
    ...
    def get(self):
        ...
        for i, c in enumerate(self.label_names):
            # scores rows are: true positives, ground-truth positives, predictions.
            # The original implementation used the latter two denominators in
            # reverse, so metric names in old logs have precision/recall swapped.
            prec = self.scores[0][i] / (self.scores[2][i] + np.finfo(float).eps)
            rec = self.scores[0][i] / (self.scores[1][i] + np.finfo(float).eps)
